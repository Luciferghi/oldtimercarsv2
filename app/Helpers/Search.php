...
class Search
{
	...
	public function fetch()
	{
		...
		unset($this->bindings['countryCode']) ;
		//dd($this->bindings);
		// Fetch Query !
		//dd($sql);
		//dd(self::query($sql, $this->bindings, 0));
		$paginator = self::query($sql, $this->bindings, 0);

		// dd($paginator);
		//$paginator = Post::orderBy('id', 'desc')->take(8)->get();
		//$paginator = Post::all();
	...
	public function countPosts()
	{
		...
		// $sql = "SELECT count(*) as total FROM Posts as x";
		...
		if (!empty($postTypes)) {
			foreach ($postTypes as $postType) {
				...
				// $sqlPostType = "SELECT count(*) as total FROM Posts as x";
